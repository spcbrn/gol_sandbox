(() => {
  ...
  GameOfLife.prototype.initializeGame = function(interval, frame_limit = 0) {
  ...
  GameOfLife.prototype._generateNewBinaryGrid = function({
    width,
    height,
    random
  }) {
  ...
  Cell.prototype._getNeighborCoordinates = function() {
    return this.nModifiers.map(modifier => {
      ...
      return nX >= 0 && nX < this.game.width && nY >= 0 && nY < this.game.height
        ? [nX, nY]
        : null;
    });
  };
  ...
  Cell.prototype._absorbLifeForce = function(life_force) {
    this.living
      ? this.bioMode
        ? this.newArt.push(life_force)
        : this.livingNeighbors++
      : this.bioMode
      ? this.dna.push(life_force)
      : this.livingNeighbors++;
  ...
  Cell.prototype._applyRules = function(done) {
    const n = this.bioMode
      ...
      : this.livingNeighbors;
    const livingNext =
      this.living === 1 ? (n <= 1 || n >= 4 ? 0 : 1) : n === 3 ? 1 : 0;
